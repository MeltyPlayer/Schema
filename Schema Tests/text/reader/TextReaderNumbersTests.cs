  ...
  internal class TextReaderNumbersTests {
    ...
    public void TestReadBytesNull() {
      ...
      Asserts.Equal(new byte?[] { 0, null, null, 255 },
                    tr.ReadBytesIncludingEmpty(new[] { "," }, TextReaderConstants.NEWLINE_STRINGS));
    }
    ...
    public void TestReadHexBytesNull() {
      ...
      Asserts.Equal(new byte?[] { 0, null, null, 255 },
                    tr.ReadHexBytesIncludingEmpty(new[] { "," }, TextReaderConstants.NEWLINE_STRINGS));
    }
    ...
    public void TestReadBytes(string inputText, byte[] expectedValues) {
      ...
      Asserts.Equal(expectedValues, tr.ReadBytes(new[] { "," }, TextReaderConstants.NEWLINE_STRINGS));
    }
    ...
    public void TestReadHexBytes(string inputText, byte[] expectedValues) {
      using var tr = TextSchemaTestUtil.CreateTextReader(inputText);
      Asserts.Equal(expectedValues, tr.ReadHexBytes(new[] { "," }, TextReaderConstants.NEWLINE_STRINGS));
    }
    ...
    public void TestReadSingles(string inputText, float[] expectedValues) {
      using var tr = TextSchemaTestUtil.CreateTextReader(inputText);
      Asserts.Equal(expectedValues, tr.ReadSingles(new[] { "," }, TextReaderConstants.NEWLINE_STRINGS));
    }
  }
}